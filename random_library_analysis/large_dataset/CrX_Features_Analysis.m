function Features = CrX_Features_Analysis(SeqAll,nupackhome,Viennahome)
    % count the library size
    smplsize = numel(SeqAll);
    
    % turn the all DNA sequence into RNA
    SeqRNA = cellfun(@(x) strrep(x,'T','U'),SeqAll,'UniformOutput',false);
    SeqRNA = cellfun(@(x) strrep(x,'t','U'),SeqRNA,'UniformOutput',false);
    ...
    %% 4.Mfe structure analysis
    % identify if Csy4 binding hairpin fold in the presumed structure;
    Core = 'CTGCCGTATAGGCAG';
    CoreStruct = rnafold(Core);
    MinSubInMfe = cell2mat(cellfun(@(x) contains(x,CoreStruct),MfeStruct,'UniformOutput',false));

    % identify the environment around Csy4 binding hairpin
    MfeStr_i = replace(MfeStruct,'(','2');
    MfeStr_i = replace(MfeStr_i,'.','1');
    MfeStr_i = cell2mat(replace(MfeStr_i,')','0'));
    MfeStr_int = zeros(size(MfeStr_i));
    for i = 1:33
        MfeStr_int(:,i) = str2num(MfeStr_i(:,i));
    end
    MfeStr_int = MfeStr_int-1;%（ . ) = 1 0 -1
    % accumulation of a complete stem-loop substructure is equal to 0
    MfeStr_lc = mat2cell(cumsum(MfeStr_int,2),ones(numel(Mfe),1),33);
    % and accumulation in a reverse order
    MfeStr_rc = mat2cell(cumsum(-fliplr(MfeStr_int),2),ones(numel(Mfe),1),33);
    % the total length of stem. If a correctly folded binding hairpin is
    % not contained in a bigger stem loop, the value should be 5
    StemLen_up = cellfun(@(x) x(14),MfeStr_lc);
    StemLen_down = cellfun(@(x) x(14),MfeStr_rc);
    % bulge size in large stem
    Bulge_up = cellfun(@(x,y) y - numel(find(x(14-y:14)))+1,...
        mat2cell(MfeStr_int,ones(numel(Mfe),1),33),...
        mat2cell(StemLen_up,ones(numel(Mfe),1)));
    Bulge_down = cellfun(@(x,y) y - numel(find(x(20:20+y)))+1,...
        mat2cell(MfeStr_int,ones(numel(Mfe),1),33),...
        mat2cell(StemLen_down,ones(numel(Mfe),1)));
    % length of adjacent stem
    StemAdj_up = zeros(size(Mfe));StemAdj_down = zeros(size(Mfe));
    for i = 1:9
        % determine if the upstream contains a continuous '('
        StemAdj_up(find(prod(MfeStr_int(:,9-i+1:9) == ones(numel(Mfe),i),2))) = i; 
        % determine if the upstream contains a continuous ')'
        StemAdj_down(find(prod(MfeStr_int(:,25:25+i-1) == -ones(numel(Mfe),i),2))) = i;
    end
    % length of adjacent loop
    LoopAdj_up = zeros(size(Mfe));LoopAdj_down = zeros(size(Mfe));
    for i = 1:9
        % determine if the upstream contains a continuous '.'
        LoopAdj_up(find(prod(MfeStr_int(:,9-i+1:9) == zeros(numel(Mfe),i),2))) = i; 
        % determine if the upstream contains a continuous '.'
        LoopAdj_down(find(prod(MfeStr_int(:,25:25+i-1) == zeros(numel(Mfe),i),2))) = i;
    end
    ...
    fMfeS = table(MinSubInMfe,...
    StemLen_up,StemLen_down,Bulge_up,Bulge_down,...
    StemAdj_up,StemAdj_down,LoopAdj_up,LoopAdj_down);
    ...
    Features = [fRawdata,fEnzemble,fKinwalker,fMfe,fMfeS,fPairWise,fSeq,fTranslation];
    save('CrX_Features.mat');
end
